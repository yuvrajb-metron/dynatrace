...
class ExtensionImpl(Extension):
    def initialize(self):
        ...
        self.security_context = attributes.get("dt.security_context")
        self.cost_center = attributes.get("dt.cost.costcenter")
        self.product = attributes.get("dt.cost.product")
        ...
        for chunk in chain(resized_chunks, failed_chunks_list):
            for event in chunk:
                if self.security_context:
                    event["dt.security_context"] = self.security_context
                if self.cost_center:
                    event["dt.cost.costcenter"] = self.cost_center
                if self.product:
                    event["dt.cost.product"] = self.product
