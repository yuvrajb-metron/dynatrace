    ...
    try:
        ...
        if plugin:  # should always be true
            ...
            plugin_cvss_base_score = (
                plugin.cvss3_base_score if plugin.cvss3_base_score is not None else plugin.cvss2_base_score
            )
            plugin_cvss_vector = (
                plugin.cvss3_vector.get("raw")
                if plugin.cvss3_vector.get("raw") is not None
                else plugin.cvss2_vector.get("raw")
            )
        ...
        base_event = {
           **event_header,
            ...
            "vulnerability.cvss.base_score": plugin_cvss_base_score,
            "vulnerability.cvss.vector": plugin_cvss_vector,
