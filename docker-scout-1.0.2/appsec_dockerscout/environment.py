...
class DynatraceEnvironmentUtils:
    """Resolve Dynatrace ActiveGate paths and API URL from on-disk configuration."""

    def __init__(self, log: logging.Logger = default_log):
        """
        Args:
            log: Logger for informational messages while scanning config files.
        """
        self.log = log

    def find_install_directory(self) -> Path:
        """
        Resolve the Dynatrace install root from ``gateway`` config under the config dir.

        Returns:
            Path to install root parsed from ``rootDir`` in gateway properties.
    ...
    def find_config_directory(self) -> Path:
        """
        Derive the config directory from ``sys.executable`` on an ActiveGate plugin host.

        Splits the executable path at ``remotepluginmodule`` and returns the prefix.

        Returns:
            Base config path (parent of ``remotepluginmodule``).
    ...
    def get_remotepluginuserconfig_conf_file(self) -> Path:
        """
        Return the path to ``remotepluginuserconfig.ini`` under the resolved config directory.
    ...
    def get_installation_config_files(self) -> list[Path]:
        """
        Return paths to ``extensions.conf`` and ``ruxitagent.conf`` under the install tree.

        Returns:
            List of two config file paths to scan for API endpoint hints.
        """
    ...
    def get_api_url(self) -> str:
        """
        Read Dynatrace environment endpoint and tenant from known config file locations.

        Returns:
            Base API URL of the form ``{endpoint}/e/{environment}``.

        Raises:
            Exception: If endpoint and tenant could not be determined from any file.
        """
        endpoint = ""
        environment = ""
